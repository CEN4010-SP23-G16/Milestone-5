        ...
        <Row className='forecast'>
          {location.state.forecast.list
            ...
            .map((data) => {
              ...
              const dayData = location.state.forecast.list.filter((d) => d.dt_txt.includes(data.dt_txt.slice(0, 10)));
              ...
              // Return the JSX to display the data
              return (
                <Col key={data.dt}>
                  <div>
                    <h3>{new Date(data.dt_txt).toLocaleDateString(undefined, { weekday: 'short' })}
                      ...
                      <br></br>
                      <p>Max: {maxTemp.toFixed()}&deg;F<br></br>Min: {minTemp.toFixed()}&deg;F</p>
                    </h3>
                  </div>
                </Col>
              )
